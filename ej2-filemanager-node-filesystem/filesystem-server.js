...
function GetFiles(req, res) {
  return new Promise((resolve, reject) => {
    fs.readdir(
      contentRootPath + req.body.path.replace(pattern, ""),
      function (err, files) {
        //handling error
        if (err) {
          console.log(err);
...
function renameFolder(req, res) {
  ...
  if (permission != null && (!permission.read || !permission.write)) {
    ...
    var oldDirectoryPath = path.join(
      contentRootPath + req.body.data[0].filterPath,
      oldName
    );
...
function getFileDetails(req, res, contentRootPath, filterPath) {
  ...
  if (req.body.names.length == 1) {
    fileDetails(
      req,
      res,
      contentRootPath + (isNamesAvailable ? req.body.names[0] : "")
    ).then((data) => {
      if (!data.isFile) {
        getFolderSize(
          req,
    ...
    fileDetails(req, res, contentRootPath + req.body.names[0]).then((data) => {
      ...
      if (filterPath == "") {
        data.location = path
          .join(rootName, filterPath)
          .substr(0, path.join(rootName, filterPath).length - 1);
      } else {
        data.location = path
          .join(rootName, filterPath)
          .substr(0, path.join(rootName, filterPath).length - 1);
        ...
        cwd.filterPath =
          req.body.data.length > 0
            ? getRelativePath(
                contentRootPath,
                contentRootPath +
                  req.body.path.substring(
                    0,
                    req.body.path.indexOf(req.body.data[0].name)
                  )
              )
            : "";
      }
  ...
  function (req, res) {
    ...
    if (
      ...
      for (var i = 0; i < fileName.length; i++) {
        fs.rename(
          "./" + fileName[i],
          path.join(contentRootPath, req.body.path + fileName[i]),
          function (err) {
            if (err) throw err;
          }
        );
...
app.post("/", function (req, res) {
  ...
  if (req.body.action == "read") {
    (async () => {
      ...
      cwdFiles.name =
        req.body.path == "/"
          ? (rootName = path.basename(contentRootPath + req.body.path))
          : path.basename(contentRootPath + req.body.path);
      var response = {};
